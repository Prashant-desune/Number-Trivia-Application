...
let getFact = () => {
  ...
  //If not empty
  if (num) {
    //Check if number lies between 0 and 999
    //if Yes fetch the fact
    if (num >= 0 && num <= 999) {
      fetchFact(num);
    }
    //If number is less than 0 or greater than 999 display error message.
    else {
      fact.style.display = "block";
      fact.innerHTML = `<p class="msg"> Please enter a number between 0 to 999.</p>`;
    }
  }
